...
class ViewController : UIViewController {
    ...
    override func viewDidLoad() {
        ...
        //a delegate to update the main map whenever a user creates an event
        EventManager.shared.eventAddedDelegate = self
    }
    ...
    func loadAndDisplayEvents(){
       ...
       DatabaseManager.shared.getAllEvents(completion: { (events) in
           //todo check about if i need the keys
           //print(events)
           for event in events.values {
                self.addNewAnnotation(event)
            }
            EventManager.shared.events = events
        })
    }
    ...
    func addNewAnnotation(_ event:Event){
        ...
        //only show future events (keep events from 24 hrs ago)
        if event.date!.timeIntervalSince1970 > Date.now.timeIntervalSince1970 - 86400{
...
extension ViewController : EventAddedDelegate {
